...
sudo pacman -Syy
sudo pacman --noconfirm -Sy python-setuptools bat alsa-plugins alsa-scarlett-gui pulseaudio-alsa lib32-alsa-plugins python-setuptools-scm lsd udiskie python-psutil volumeicon cbatticon networkmanager curl feh nano git netctl dialog dhclient git vlc xterm firefox rofi which nitrogen picom gvfs ttf-dejavu ttf-liberation noto-fonts geeqie pavucontrol pamixer ranger glib2 arandr udiskie ntfs-3g network-manager-applet volumeicon cbatticon xorg-xinit base-devel thunar gvfs lxappearance alacritty neofetch htop unzip alsa-utils keepass opera code rofi
